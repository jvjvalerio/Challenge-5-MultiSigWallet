...
class L1ToL2Message {
    constructor(chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId // TODO: remove this after migration
    ) {
        ...
        if ((0, lib_1.isDefined)(retryableCreationId)) {
    ...
    static fromTxComponents(l2SignerOrProvider, chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId // TODO: remove this after migration
    ) {
        return signerOrProvider_1.SignerProviderUtils.isSigner(l2SignerOrProvider)
            ? new L1ToL2MessageWriter(l2SignerOrProvider, chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId)
            : new L1ToL2MessageReader(l2SignerOrProvider, chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId);
    }
}
exports.L1ToL2Message = L1ToL2Message;
class L1ToL2MessageReader extends L1ToL2Message {
    constructor(l2Provider, chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId // TODO: remove this after migration
    ) {
        ...
        while (fromBlock.number < maxBlock) {
            ...
            if (toBlock.timestamp > timeout) {
                ...
                while (queriedRange.length > 0) {
                    ...
                    const keepaliveEvents = await eventFetcher.getEvents(constants_1.ARB_RETRYABLE_TX_ADDRESS, ArbRetryableTx__factory_1.ArbRetryableTx__factory, contract => contract.filters.LifetimeExtended(this.retryableCreationId), { fromBlock: blockRange.from, toBlock: blockRange.to });
...
class L1ToL2MessageWriter extends L1ToL2MessageReader {
    constructor(l2Signer, chainId, sender, messageNumber, l1BaseFee, messageData, retryableCreationId // TODO: remove this after migration
    ) {
